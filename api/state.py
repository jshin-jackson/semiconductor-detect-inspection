...
class AppState:
    ...
    def initialize(self, config_path: str) -> None:
        ...
        if ckpt:
            logger.info("Checkpoint found — loading model: %s", ckpt)
            self.load_model(ckpt)
        else:
            logger.warning("No checkpoint found in %s — POST /train to train first",
                           self.config["output"]["weights_dir"])
    ...
    def load_model(self, ckpt_path: str | None) -> None:
        ...
        if not ckpt_path or not os.path.isfile(ckpt_path):
            logger.warning("load_model: checkpoint not found: %s", ckpt_path)
            return
        ...
        try:
            ...
            logger.info("Model loaded: %s  threshold=%.2f", ckpt_path, self._raw_threshold)
